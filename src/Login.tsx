...
export function Login (): ReactElement {
  ...
  async function databaseask (): Promise<void> {
    setLoader(true)
    const answer = await fetch('http://loginpage:5001/login', {
      method: 'POST',
    ...
    if (answer.status === 200) {
      localStorage.setItem('JsonWebToken', 'Bearer' + String(result.jwt))
      navigate(`/user/${Number(result.id)}`)
    } else {
      setError('error')
